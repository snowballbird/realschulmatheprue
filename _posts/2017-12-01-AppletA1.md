...
board.create('text', [6,15, function(){return 'DCn(&phi;)='+Math.round(100*Math.sqrt((C.X()-D.X())*(C.X()-D.X())+(C.Y()-D.Y())*(C.Y()-D.Y())))/100+'LE'}])
board.create('text', [6,15, function(){return 'SAn(&phi;)='+Math.round(100*Math.sqrt((A.X()-S.X())*(A.X()-S.X())+(A.Y()-S.Y())*(A.Y()-S.Y())))/100+'LE'}])
