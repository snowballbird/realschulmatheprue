...
const board = JXG.JSXGraph.initBoard('05066aa6-c9fb-4e0e-bddf-02a6e456646e', {
 ...
 var C = board.create('point', [0,4], { fixed: true, name: 'C', fontsize: 15, size:2, label:{fontsize:15} });
 
  var M = board.create('point', [0,-1], { fixed: true, name: 'M', fontsize: 15, size:2, label:{fontsize:15} });
...
var fix = board.create('functiongraph', [x => -1], {visible: false})
  ...
  -1, fix], {name: 'B', fixed: false, color: 'orange', size: 4, size:2, label:{fontsize:15}});
  
  var A = board.create('point', [  function() { return -1 * B.X(); },
  -1],
  {name: 'A', fixed: true, fontsize: 15, size:2, label:{fontsize:15}});  
