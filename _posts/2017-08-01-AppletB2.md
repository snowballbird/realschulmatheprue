...
board.create('text', [8,10, function(){return '&phi; = '+ JXG.toFixed(a.Value()*180/Math.PI, 2);}], {fontsize: 18, fixed:true});

board.create('text', [8,8.5, function(){return '|<span style="border-top:1px solid">KP</span>| = ' + Math.round(100*Math.sqrt((K.X()-KPC.X())*(K.X()-KPC.X())+(K.Y()-KPC.Y())*(K.Y()-KPC.Y())))/100;}], {fontsize: 18, fixed:true});
